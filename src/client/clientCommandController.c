...
static void client_command_controller_load_queries(CommandController controllers[], char descriptions[]) {
    ...
    controllers[index + CQ_LIST_USERS].receiver = list_users_reciever;
    ...
    controllers[index + CQ_TOTAL_HISTORIC_CONNECTIONS].receiver = total_historic_connections_reciever;
    ...
    controllers[index + CQ_CURRENT_CONNECTIONS].receiver = current_connections_reciever;
    descriptions[index + CQ_CURRENT_CONNECTIONS] = "Get Current Connections";

    // X'03'   Get Max. Concurrent Connections
    controllers[index + CQ_MAX_CURRENT_CONECTIONS].sender = max_current_conections_sender;
    controllers[index + CQ_MAX_CURRENT_CONECTIONS].receiver = max_current_conections_reciever;
    ...
    controllers[index + CQ_TOTAL_BYTES_SENT].receiver = total_bytes_sent_reciever;
    ...
    controllers[index + CQ_TOTAL_BYTES_RECEIVED].receiver = total_bytes_received_reciever;
    ...
    controllers[index + CQ_CONNECTED_USERS].receiver = connected_users_reciever;
    ...
    controllers[index + CQ_USER_COUNT].receiver = user_count_reciever;
    ...
    controllers[index + CQ_BUFFER_SIZES].receiver = buffer_sizes_reciever;
    ...
    controllers[index + CQ_SELECTOR_TIMEOUT].receiver = selector_timeout_reciever;
    ...
    controllers[index + CQ_CONNECTION_TIMEOUT].receiver = connection_timeout_reciever;
    ...
    controllers[index + CQ_USER_TOTAL_CONCURRENT_CONNECTIONS].receiver = user_total_concurrent_connections_reciever;
...
static void client_command_controller_load_modifications(CommandController controllers[], char descriptions[]) {
    ...
    controllers[index + CM_ADD_USER].receiver = add_user_reciever;
    descriptions[index + CM_ADD_USER] = "Add User";

    // X'01'   Remove User
    controllers[index + CM_REMOVE_USER].sender = remove_user_sender;
    controllers[index + CM_REMOVE_USER].receiver = remove_user_reciever;
    ...
    controllers[index + CM_TOGGLE_PASSWORD_SPOOFING].receiver = toggle_password_spoofing_reciever;
    ...
    controllers[index + CM_TOGGLE_CONNECTION_CLEAN_UN].receiver = toggle_connection_clean_up_reciever;
    ...
    controllers[index + CM_SET_BUFFER_SIZE].receiver = set_buffer_size_reciever;
    ...
    controllers[index + CM_SET_SELECTOR_TIMEOUT].receiver = set_selector_timeout_reciever;
    ...
    controllers[index + CM_SET_CONNECTION_TIMEOUT].receiver = set_connection_timeout_reciever;
